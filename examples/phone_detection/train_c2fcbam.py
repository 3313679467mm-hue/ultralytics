...
if __name__ == "__main__":
    ...
    # C2fCBAM 配置文件路径
    model_yaml_path = os.path.join(script_dir, "..", "..", "ultralytics", "cfg", "models", "v8", "yolov8-c2fcbam.yaml")

    # 使用相同的预训练权重（保证公平对比）
    weights_path = os.path.join(script_dir, "..", "..", "weights", "best_modified.pt")

    print("=" * 60)
    print("C2fCBAM 模块测试（CBAM 注意力机制）")
    print("=" * 60)
    ...
    # 开始训练
    print("\n开始训练 C2fCBAM 模型（最终优化版 - 只在最后一层用注意力）...")
    results = model.train(
        # 数据集配置
        data=data_path,
        # 训练参数
