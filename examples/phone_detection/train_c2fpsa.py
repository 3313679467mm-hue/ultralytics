...
if __name__ == "__main__":
    ...
    # C2fPSA 配置文件路径
    model_yaml_path = os.path.join(script_dir, "..", "..", "ultralytics", "cfg", "models", "v8", "yolov8-c2fpsa.yaml")

    # 使用相同的预训练权重（保证公平对比）
    weights_path = os.path.join(script_dir, "..", "..", "weights", "best_modified.pt")

    print("=" * 60)
    print("C2fPSA 模块测试（PSA 注意力机制）")
    print("=" * 60)
    ...
    # 开始训练
    print("\n开始训练 C2fPSA 模型（最终优化版 - 只在最后一层用注意力）...")
    results = model.train(
        # 数据集配置
        data=data_path,
        # 训练参数
        ...
        imgsz=640,
        device=0,
        batch=4,  # 与 C2fSE 保持一致
        # 结果保存
        name="phone_detection_c2fpsa_final",
        project="runs",
        # 数据增强
        hsv_h=0.025,
        hsv_s=0.8,
