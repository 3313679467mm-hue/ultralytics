...
class SimSPPF(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Apply simplified pooling operations to input and return concatenated feature maps."""
        y = [self.cv1(x)]
...
class C2fLightConv(nn.Module):
    ...
    def __init__(self, c1: int, c2: int, n: int = 1, shortcut: bool = False, g: int = 1, e: float = 0.5):
        """Initialize C2fLightConv module.

        Args:
            c1 (int): Input channels.
            c2 (int): Output channels.
            n (int): Number of LightConv blocks.
            shortcut (bool): Whether to use shortcut connections.
            g (int): Groups for convolutions.
            e (float): Expansion ratio.
        """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Forward pass through C2fLightConv layer."""
        y = list(self.cv1(x).chunk(2, 1))
        y.extend(m(y[-1]) for m in self.m)
        return self.cv2(torch.cat(y, 1))
...
class C2fSE(C2f):
    """C2f module with Squeeze-and-Excitation (SE) attention.

    在 C2f 的基础上融入 SE 注意力机制，增强通道特征表达能力。
    """

    def __init__(self, c1: int, c2: int, n: int = 1, shortcut: bool = False, g: int = 1, e: float = 0.5):
        """Initialize C2fSE module.
        ...
        Args:
            ...
            e (float): Expansion ratio.
        """
        # 先计算 self.c，确保至少为 1，避免后续空张量初始化错误
        self.c = max(int(c2 * e), 1)  # hidden channels，至少为 1
        # 确保 c2 至少为 1，避免空张量初始化错误
        c2 = max(c2, 1)
        # 调用父类初始化之前手动设置必要的属性
        nn.Module.__init__(self)
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv(max((2 + n) * self.c, 1), c2, 1)  # optional act=FReLU(c2)
        self.m = nn.ModuleList(Bottleneck(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))
        # 初始化 SE 模块，确保 se_channels 至少为 1
        se_channels = max(self.c, 1)
        self.se = SE(se_channels)  # 对 Bottleneck 的输出应用 SE 注意力
